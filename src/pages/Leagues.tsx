...
const LeaguesPage = () => {
  ...
  const handleLeagueClick = (leagueId: string) => {
    console.log('Clicking league:', leagueId); // Debug log
    setSelectedLeague(leagueId);

    // Map league IDs to the correct route format (country/league-name)
    ...
    const routePath = leagueRouteMapping[leagueId];
    if (routePath) {
      // Navigate to the league details page with correct format
      navigate(`/leagues/${routePath}`);
    } else {
      console.error('No route mapping found for league:', leagueId);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#111111] to-[#0f0f0f]">
      <MetaTags
        title="Football Leagues & Competitions | livescoreresult"
        description="Browse all major football leagues and competitions worldwide. Find fixtures, standings, and results from Premier League, La Liga, Serie A, Bundesliga and more."
        url="/leagues"
      />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filteredLeagues.map((league) => (
                <div
                  key={league.id}
                  className={cn(
                    "cursor-pointer transition-all duration-200 border border-white/10 bg-[#1a1a1a] hover:bg-[#2a2a2a] rounded-lg",
                ...
                >
                  <div className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      {league.isPopular && (
                        <div className="inline-flex items-center rounded-full border px-2.5 py-0.5 font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent hover:bg-secondary/80 bg-[#ff6b00]/20 text-[#ff6b00] text-xs">
                          ★
                        </div>
                      )}
                    </div>
